...
for fasta_file in "$INPUT_DIR"/*"$FILE_EXT"; do
    # Extract base name without extension
    DATA=$(basename "$fasta_file" "$FILE_EXT")
    FOLDER="$INPUT_DIR"
    
    # Define per-file result folders
    ...
    ###### PREPARE FILES #####
    printf "\nClassifying assembly: $fasta_file \n\n"
    ...
    if [ ! -f "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_KMERCLASS}bp$FILE_EXT" ] && [ ! -f "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT" ]; then
        ...
        seqkit seq "$fasta_file" -m $MINSIZE_CONTIGS_KMERCLASS -o "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_KMERCLASS}bp$FILE_EXT" 
        ...
        if [ $MINSIZE_CONTIGS_KMERCLASS != $MINSIZE_CONTIGS_DBCLASS ]; then
            seqkit seq "$fasta_file" -m $MINSIZE_CONTIGS_DBCLASS -o "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT"
    ...
    if [ ! -f "$FOLDER/$RESULTS_FOLDER/deepmicroclass-results_min${MINSIZE_CONTIGS_KMERCLASS}bp/${DATA}_min${MINSIZE_CONTIGS_KMERCLASS}bp${FILE_EXT}_pred_onehot_hybrid.class.tsv" ]; then
        ...
        $RSCRIPT_PATH "$EUKSEMBLE_INSTALLATION_FOLDER/Scripts/DeepMicroClass_predictionAssignment_extInput.R" \
            "$FOLDER/$RESULTS_FOLDER/deepmicroclass-results_min${MINSIZE_CONTIGS_KMERCLASS}bp" \
            "${DATA}_min${MINSIZE_CONTIGS_KMERCLASS}bp$FILE_EXT_pred_onehot_hybrid.tsv"
    else
    ...
    if [ $INCLUDE_NA == "TRUE" ]; then
        seqkit grep -f "$FOLDER/$RESULTS_FOLDER/${DATA}.EUK_NA_contigsIDs_KmerMajority_min${MINSIZE_CONTIGS_KMERCLASS}bp_${DB_CLASSIFIER^^}_min${MINSIZE_CONTIGS_DBCLASS}bp.txt" \
            "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT" \
            -o "$FOLDER/$RESULTS_FOLDER/${DATA}.EUK_NA_KmerMajority_min${MINSIZE_CONTIGS_KMERCLASS}bp_${DB_CLASSIFIER^^}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT"
    else
        seqkit grep -f "$FOLDER/$RESULTS_FOLDER/${DATA}.EUK_contigsIDs_KmerMajority_min${MINSIZE_CONTIGS_KMERCLASS}bp_${DB_CLASSIFIER^^}_min${MINSIZE_CONTIGS_DBCLASS}bp.txt" \
            "$FOLDER/$RESULTS_FOLDER/${DATA}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT" \
            -o "$FOLDER/$RESULTS_FOLDER/${DATA}.EUK_KmerMajority_min${MINSIZE_CONTIGS_KMERCLASS}bp_${DB_CLASSIFIER^^}_min${MINSIZE_CONTIGS_DBCLASS}bp$FILE_EXT"
    fi
